...
def init_db():
    ...
    # 5. Alternatives / Resolutions Table
    cursor.execute("""
...
class DatabaseStore:
    ...
    @staticmethod
    def save_revision(revision: Revision, raw_text: str = ""):
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute(
    ...
    @staticmethod
    def get_project_revisions(project_id: str) -> List[Revision]:
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
    ...
    @staticmethod
    def save_claims(claims: List[Claim]):
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        for claim in claims:
            cursor.execute(
                "INSERT OR REPLACE INTO claims (claim_id, item_id, item_string, item_type, revision_id, state, outcome, scope, queries, search_ids, evidence, human_disposition, disposition_note, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    claim.claim_id,
                    claim.item_id,
                    claim.item_string,
    ...
    @staticmethod
    def get_claims_for_revision(revision_id: str) -> List[Claim]:
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute("SELECT claim_id, item_id, item_string, item_type, revision_id, state, outcome, scope, queries, search_ids, evidence, human_disposition, disposition_note, created_at, updated_at FROM claims WHERE revision_id = ?", (revision_id,))
        rows = cursor.fetchall()
        conn.close()
        claims = []
        for row in rows:
            claims.append(Claim(
                ...
                human_disposition=HumanDisposition(row[11]) if row[11] else None,
                disposition_note=row[12],
                created_at=row[13],
                updated_at=row[14]
            ))
        return claims
    ...
    @staticmethod
    def save_egress_log(query: str, allowed: bool, provenance: List[str], search_id: str, timestamp: str):
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
    ...
    @staticmethod
    def get_egress_logs() -> List[Dict[str, Any]]:
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
    ...
    @staticmethod
    def save_alternative(claim_id: str, original_name: str, alternative_name: str, outcome: str, evidence: List[Any]):
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
    ...
    @staticmethod
    def get_alternatives(claim_id: str) -> List[Dict[str, Any]]:
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
